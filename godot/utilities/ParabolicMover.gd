...
func start(target_position: Vector2, arc_time: float):
	...
	self.fire_velocity = self.__calculate_initial_velocity(self.get_parent().position, self.target_position)
...
func __has_reached_target():
	if self.already_reached_target:
		return false
	var dist = self.get_parent().position.distance_squared_to(self.target_position)
	if dist <= HIT_DIST:
		return true
	
	var next_pos = self.get_parent().position + self.fire_velocity.normalized()
	var next_dist = next_pos.distance_squared_to(self.target_position)
